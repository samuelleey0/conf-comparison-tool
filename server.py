...
def api_melbourne_send():
    try:
        ...
        result = _export_to_melbourne(payload)
...
def _melbourne_quote_toml(value):
    return '"' + str(value).replace("\\", "\\\\").replace('"', '\\"') + '"'


def _melbourne_render_toml_value(value):
    if isinstance(value, str):
        return _melbourne_quote_toml(value)
    if isinstance(value, bool):
        return "true" if value else "false"
    if isinstance(value, (int, float)):
        return str(value)
    if isinstance(value, list):
        return "[" + ", ".join(_melbourne_render_toml_value(item) for item in value) + "]"
    return _melbourne_quote_toml(value)


def _melbourne_render_toml(data):
    lines = []
    for section, values in data.items():
        ...
        if simple_values:
            ...
            for key, value in simple_values.items():
                lines.append(f"{key} = {_melbourne_render_toml_value(value)}")
            lines.append("")
...
def _melbourne_slug(value):
    cleaned = re.sub(r"[^A-Za-z0-9._-]+", "-", str(value or "").strip())
    cleaned = re.sub(r"-{2,}", "-", cleaned).strip("-._")
    return cleaned or "melbourne-export"


def _melbourne_safe_int(value, default, minimum=None):
    try:
        parsed = int(value)
    except (TypeError, ValueError):
        parsed = default
...
def _melbourne_parse_csv(value, default=None):
    items = [item.strip() for item in str(value or "").split(",") if item.strip()]
    return items or list(default or [])


def _melbourne_parse_int_csv(value, default):
    parsed = []
    for item in _melbourne_parse_csv(value, default):
        try:
...
def _melbourne_derive_exam_fields(exam_name):
...
def _melbourne_session_path(payload):
...
def _melbourne_student_dirs(collection_root):
    return sorted(path for path in collection_root.iterdir() if path.is_dir() and not path.name.startswith("."))


def _melbourne_discover_devices(collection_root):
    devices = set()
    for student_dir in _melbourne_student_dirs(collection_root):
        devices.update(path.name for path in student_dir.iterdir() if path.is_dir() and not path.name.startswith("."))
    if not devices:
        raise ValueError(f'No device folders found below "{collection_root}".')
    return sorted(devices)


def _melbourne_infer_device_type(collection_root, device_name):
...
def _melbourne_write_exam_config(output_file, collection_root, payload):
    exam_name = str(payload.get("exam_name") or "").strip()
    if not exam_name:
        raise ValueError("Exam name is required.")
    inferred_unitcode, inferred_shortname = _melbourne_derive_exam_fields(exam_name)
    ...
    timeout = _melbourne_safe_int(payload.get("timeout"), 180, 1)
    scheme_values = _melbourne_parse_csv(payload.get("scheme_values"))
    ...
    lines = [
        "[details]",
        f"name = {_melbourne_quote_toml(exam_name)}",
        f"semester = {_melbourne_quote_toml(semester)}",
        f"unitcode = {_melbourne_quote_toml(unitcode)}",
        f"shortname = {_melbourne_quote_toml(shortname)}",
        "",
        "[collect]",
        f"timeout = {timeout}",
        "",
    ]
    for device in _melbourne_discover_devices(collection_root):
        exam_device_name = device_names.get(device, device)
        device_type = device_types.get(device, _melbourne_infer_device_type(collection_root, device))
        lines.extend(
            [
                f"[collect.{exam_device_name}]",
                f"type = {_melbourne_quote_toml(device_type)}",
                "",
            ]
        )
    lines.extend(
        [
            "[options]",
            "scheme = [" + ", ".join(_melbourne_quote_toml(value) for value in scheme_values) + "]",
            "",
        ]
    )
    ...
    return {
        ...
        "devices": {
            device_names.get(device, device): {
                "type": device_types.get(device, _melbourne_infer_device_type(collection_root, device)),
...
def _melbourne_log_filename_to_command(filename):
...
def _melbourne_commands_from_collection(collection_root, source_device_name):
    commands = []
    seen = set()
    for log_file in sorted(collection_root.glob(f"*/{source_device_name}/*")):
        if not log_file.is_file():
            continue
        command = _melbourne_log_filename_to_command(log_file.name)
...
def _melbourne_write_master_solution(output_file, collection_root, config):
    rubric_name = str(config.get("rubric_name") or "Major_Minor").strip() or "Major_Minor"
    maximum = _melbourne_safe_int(config.get("maximum_marks"), 100, 1)
    minor_pen = _melbourne_parse_int_csv(config.get("minor_penalties"), [10, 20, 30, 40])
    ...
    for device_name, details in devices.items():
        ...
        commands = _melbourne_commands_from_collection(collection_root, source_folder)
        if not commands:
...
def _melbourne_read_hostname(device_dir):
    candidates = [
        device_dir / "show_running-config.txt",
        device_dir / "show_run.txt",
        device_dir / "sh_run",
        device_dir / "show_running-config",
    ]
    for candidate in candidates:
...
def _melbourne_extract_rubrics(solution_file):
...
def _melbourne_write_student_files(export_root, exam_name, solution_file, student_schemes):
    finalised = 0
    missing_schemes = []
    for student_dir in _melbourne_student_dirs(export_root):
        ...
        exam_information = {
            ...
            "Rubrics": _melbourne_extract_rubrics(solution_file),
        }
        with (student_dir / "exam_info.toml").open("w", encoding="utf-8") as file:
            file.write(_melbourne_render_toml(exam_information))
        finalised += 1
    return finalised, missing_schemes


def _melbourne_zip_folder(source_dir, zip_path):
...
def _export_to_melbourne(payload):
    collection_root = _melbourne_session_path(payload)
    source_students = _melbourne_student_dirs(collection_root)
    ...
    export_base_name = _melbourne_slug(f"{classroom}-{tutor_name}-{export_time}")
    ...
    exam_details = _melbourne_write_exam_config(exam_config_file, export_root, payload)
    rubric_details = _melbourne_write_master_solution(
    ...
    if not isinstance(raw_student_schemes, dict):
        raw_student_schemes = {}
    finalised, missing_schemes = _melbourne_write_student_files(
        export_root,
    ...
    _melbourne_zip_folder(export_root, zip_path)
